    ...
    */
    ...
    handleIFrameHeight: function () {
        ...
        if (window.FORM_MODE === 'cardform') {
            ...
            var emInPx = document.getElementsByClassName('jfCard')[0].getStyle("fontSize").slice(0,-2);
...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    initMultipleUploads: function () {
        ...
        $$('.form-upload-multiple').each(function (file) {
            ...
            if (m = parent.next(".cancelText")) {
                cancelText = m.innerText;
            }
            if (m = parent.next(".ofText")) {
                ofText = m.innerText;
            }
    ...
    displayTimeRangeDuration: function (id) {
        var displayDuration = function () {
            if ($('input_' + id + '_hourSelectRange')) {
                ...
                if (window.FORM_MODE === 'cardform') {
                    if (lab == '_ampmRange') {
                        durationLabel = $$('div[for=input_' + id + lab + ']').first();
                    } else {
                    ...
                    $$("#id_" + data + ' input[type="radio"]').each(function (rad, i) {
                        if (rad.checked) {
                            if (rad.readAttribute('data-calcvalue')) {
                                ...
                                if (typeof FormTranslation !== 'undefined' && JotForm.getOptionOtherInput(rad)) {
    ...
    setMatrixLayout: function(id, passive, mobileActiveQuestionOrder) {
        ...
        if ($(matrix).select('.forMobile').length > 0) {
            ...
            var setActiveQuestion = function (activeQuestionOrder) {
                ...
                $(matrix).select('.jfMatrix-progress .forMatrixPrev')[0].disabled = !isBackEnabled;
                $(matrix).select('.jfMatrix-progress .forMatrixNext')[0].disabled = !isNextEnabled;
                // set question number
            ...
            $(matrix).select('.jfMatrixProgress-button.forMatrixNext').length > 0 && $(matrix).select('.jfMatrixProgress-button.forMatrixNext')[0].observe("click", handleNextButtonClick);
            $(matrix).select('.jfMatrixProgress-button.forMatrixPrev').length > 0 && $(matrix).select('.jfMatrixProgress-button.forMatrixPrev')[0].observe("click", handlePrevButtonClick);
    ...
    setRatingItemsChecked: function(id, value, ratingBefore) {
        ...
        if(ratingBefore < value || ratingBefore === null) {
        ...
        if(ratingBefore && ratingBefore > value) {
    ...
    toggleImageOverlay: function(id, src) {
        var imgWrapper = document.getElementById("id_" + id);
        var btn = imgWrapper.select('.js-openNewTab')[0];
        var closeBtn = document.querySelector('.jfOverlay-close');
        
        var overlayWrapper = $$('.js-overlayWrapper')[0];

        var toggleOverlay = function(isVisible) {
            var overlayImg = overlayWrapper.select('.js-overlayImage')[0];
            overlayImg.src = !isVisible ? src : '';

            if(!isVisible) {
                overlayWrapper.addClassName('isVisible');
            } else {
                overlayWrapper.removeClassName('isVisible');
            }
        }
        ...
        $(overlayWrapper).observe('click', function(e) {
            e.stop();

            if (!$(e.target).hasClassName('js-overlayImage')) {
                toggleOverlay(true);
            }
        })
    },
